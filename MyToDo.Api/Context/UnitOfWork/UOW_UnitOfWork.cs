...
{
    /// <summary>
    /// Represents the default implementation of the <see cref="UOW_IUnitOfWork"/> and <see cref="IUnitOfWork{TContext}"/> interface.
    /// </summary>
    /// <typeparam name="TContext">The type of the db context.</typeparam>
    public class UOW_UnitOfWork<TContext> : UOW_IRepositoryFactory, IUnitOfWork<TContext>, UOW_IUnitOfWork where TContext : DbContext
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UOW_UnitOfWork{TContext}"/> class.
        /// </summary>
        /// <param name="context">The context.</param>
        public UOW_UnitOfWork(TContext context)
        {
        ...
        public UOW_IRepository<TEntity> GetRepository<TEntity>(bool hasCustomRepository = false) where TEntity : class
        {
            ...
            {
                var customRepo = _context.GetService<UOW_IRepository<TEntity>>();
                if (customRepo != null)
                {
                    return customRepo;
                }
            ...
            {
                repositories[type] = new UOW_Repository<TEntity>(_context);
            }

            return (UOW_IRepository<TEntity>)repositories[type];
        }
        ...
        /// </summary>
        /// <param name="ensureAutoHistory"><c>True</c> if save changes ensure auto record the change history.</param>
        /// <param name="unitOfWorks">An optional <see cref="UOW_IUnitOfWork"/> array.</param>
        /// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous save operation. The task result contains the number of state entities written to database.</returns>
        public async Task<int> SaveChangesAsync(bool ensureAutoHistory = false, params UOW_IUnitOfWork[] unitOfWorks)
        {
